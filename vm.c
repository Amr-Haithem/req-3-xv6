...
int getLRU(){
  //get the least recently used page to page out
  int i; 
  int pageIndex = -1;
  uint minAccess = 0xffffffff;
  //min Access holds the count of the least accessed page

  for (i = 0; i < MAX_PSYC_PAGES; i++) {
    //here it iterates on the pages to see if a page is used
    //and if a page is lest recently used then update the pageIndex by it
    if (proc->ramCtrlr[i].state == USED && proc->ramCtrlr[i].accessCount <= minAccess) {
          minAccess = proc->ramCtrlr[i].accessCount;
          pageIndex = i;          
    }
  }
  //reutrn the least recently used
  return pageIndex;
}
...
//end of custommed
 ...
 //if all full make lru if not add to ram ctrlr
 //allocuvm is my main
int
